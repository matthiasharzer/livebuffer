...
| GET    | `/api/v1/{username}/download?stream_id=<stream_id>`                               | Downloads a specific livestream of the user by its ID as a file.                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| GET    | `/api/v1/{username}/clip?stream_id=<stream_id>&start=<start_time>&end=<end_time>` | Creates a clip from a specific livestream of the user by providing the start and end times. <br/>`start` and `end` must be in a format parsable by [Go's `time.ParseDuration`](https://pkg.go.dev/time#ParseDuration) (e.g., `10m` for 10 minutes). Valid units are "ns", "us" (or "µs"), "ms", "s", "m", "h". <br/> If `end` is greater than the stream's duration, the clip will be truncated to the end of the stream. <br/>If `start` is greater than the stream's duration, an empty file will be returned. |
| GET    | `/api/v1/{username}/live`                                                         | Streams the currently live stream of the user if available (restream). If no stream is live, a 404 response will be returned.                                                                                                                                                                                                                                                                                                                                                                                    |
| GET    | `/{username}/live`                                                                | Serves an HTML page which embeds the livestream of the user, if available.                                                                                                                                                                                                                                                                                                                                                                                                                                       |
